...
def precondition_tensors(tens, mask, fw=np):
  # get rid of nans, poor conditioned tensors
  # Only keep elements of mask that are large enough to take accurate derivatives
  tens[fw.isnan(tens)] = 0

def compute_g_derivs_new(g, g_inv, sqrt_det_g, bdry_idx, bdry_map, fw=np):
  xsz = g.shape[0]
  ysz = g.shape[1]
  zsz = g.shape[2]
  gradG = fw.zeros((xsz,ysz, zsz, 3, 3, 3),dtype=fw.double)
  grad_g_inv = fw.zeros((xsz,ysz, zsz, 3, 3, 3),dtype=fw.double)
  grad_sqrt_det_g = fw.zeros((xsz,ysz,zsz,3),dtype=fw.double)
...
def compute_T_derivs_new(TDirx, TDiry, TDirz, bdry_idx, bdry_map, fw=np):
  xsz = TDirx.shape[0]
  ysz = TDirx.shape[1]
  zsz = TDirx.shape[2]
  gradTx_delx = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTx_dely = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTx_delz = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTy_delx = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTy_dely = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTy_delz = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTz_delx = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTz_dely = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  gradTz_delz = fw.zeros((xsz,ysz,zsz), dtype=fw.double)
  ...
  for btype, bnum in bdry_map.items():
    ...
    if btype == "outside":
      ...
      gradTx_delx[b_idx[0], b_idx[1],b_idx[2]], gradTy_delx[b_idx[0], b_idx[1],b_idx[2]], gradTz_delx[b_idx[0], b_idx[1],b_idx[2]] = xeigvdiff(TDirx, b_idx, fw)
      gradTx_dely[b_idx[0], b_idx[1],b_idx[2]], gradTy_dely[b_idx[0], b_idx[1],b_idx[2]], gradTz_dely[b_idx[0], b_idx[1],b_idx[2]] = yeigvdiff(TDiry, b_idx, fw)
      gradTx_delz[b_idx[0], b_idx[1],b_idx[2]], gradTy_delz[b_idx[0], b_idx[1],b_idx[2]], gradTz_delz[b_idx[0], b_idx[1],b_idx[2]] = zeigvdiff(TDirz, b_idx, fw)
...
def compute_nabla_derivs_new(nabla_TT, sqrt_det_nabla_TT, bdry_idx, bdry_map, fw=np):
  xsz = nabla_TT.shape[0]
  ysz = nabla_TT.shape[1]
  zsz = nabla_TT.shape[2]
  grad_nabla_TT = fw.zeros((xsz,ysz, zsz, 3, 3),dtype=fw.double)
  grad_sqrt_det_nabla_TT = fw.zeros((xsz,ysz,zsz, 3, 3),dtype=fw.double)
...
def compute_alpha_derivs(alpha, bdry_idx, bdry_map, fw=np):
  xsz = alpha.shape[0]
  ysz = alpha.shape[1]
  zsz = alpha.shape[2]
  alpha_gradX = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  alpha_gradY = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  alpha_gradZ = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
...
def neumann_conditions_rhs(nabla_TT, g, grad_det_g_x_g_inv, grad_g_inv_x_det_g, det_g_x_g_inv, sqrt_det_g, bdry_idx, bdry_map, fw=np):
  xsz = nabla_TT.shape[0]
  ysz = nabla_TT.shape[1]
  zsz = nabla_TT.shape[2]
  bdry_term = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  n = fw.zeros((3),dtype=fw.double)
  ...
  for btype, bnum in bdry_map.items():
    ...
    nmag = fw.sqrt(n[0]*n[0] + n[1]*n[1] + n[2]*n[2])
...
# Note, need to fix with args prior to making a LinearOperator
  ...
  fw=args['fw']
  alpha = fw.zeros((args['xsz'],args['ysz'],args['zsz']),dtype=fw.double)
  alpha[args['mask']>0] = x

  gradX, gradY, gradZ = compute_alpha_derivs(alpha, args['bdry_idx'], args['bdry_map'])
  
  div = fw.zeros((args['xsz'],args['ysz'],args['zsz']),dtype=fw.double)

  gradXX = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  gradXY = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  gradXZ = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  gradYY = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  gradYZ = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  gradZZ = fw.zeros((args['xsz'],args['ysz'],args['zsz']), dtype=fw.double)
  ...
  for btype, bnum in args['bdry_map'].items():
    ...
    if btype[0:8] == "interior":
      ...
      # else:
      #   if "interiorrearbottom" in btype:
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_rearbottom_idx_3d(alpha, b_idx)
      #   elif "interiorreartop" in btype:
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_reartop_idx_3d(alpha, b_idx)
      #   elif "interiorfrontbottom" in btype:
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_frontbottom_idx_3d(alpha, b_idx)
      #   elif "interiorfronttop" in btype:
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradyz_fronttop_idx_3d(alpha, b_idx)
      #   else:
      #      gradYZ[b_idx[0], b_idx[1],b_idx[2]] = 0
      #   if "interiorrearleft" in btype:
      #     gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_rearleft_idx_3d(alpha, b_idx)
      #   elif "interiorrearright" in btype:
      #     gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_rearright_idx_3d(alpha, b_idx)
      #   elif "interiorfrontleft" in btype:
      #     gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_frontleft_idx_3d(alpha, b_idx)
      #   elif "interiorfrontright" in btype:
      #     gradXZ[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxz_frontright_idx_3d(alpha, b_idx)
      #   else:
      #     gradYZ[b_idx[0], b_idx[1],b_idx[2]] = 0
      #   if "interiorbottomleft" in btype:
      #     gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_bottomleft_idx_3d(alpha, b_idx)  
      #   elif "interiorbottomright" in btype:
      #     gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_bottomright_idx_3d(alpha, b_idx)  
      #   elif "interiortopleft" in btype:
      #     gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_topleft_idx_3d(alpha, b_idx)  
      #   elif "interiortopright" in btype:
      #     gradXY[b_idx[0], b_idx[1],b_idx[2]] = diff.gradxy_topright_idx_3d(alpha, b_idx)
      #   else:
      #     gradXY[b_idx[0], b_idx[1],b_idx[2]] = 0  
      ...
      if "shiftx" in btype:
        ...
        if ("shifty" in btype):
          ...
          if "tore" in btype:
            xydiff =  (-0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                      + 0.5*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1] - 0.5*alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+1])
          elif "tof" in btype:
            xydiff =  (-0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1] + 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1]
                      + 0.5*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]-1] - 0.5*alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]-1])
        ...
        if ("shifty" in btype) or ("shiftbottom" in btype) or ("shifttop" in btype):
          ...
          #print('in else case 1, btype=', btype)
          xdiff = None # Set to None, because this term is in RHS Neumann condition
          if "tore" in btype or "rear" in btype:
            xydiff = (0.25*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]+1] - 0.25*alpha[b_idx[0]+1,b_idx[1]-1,b_idx[2]+1]
                    - 0.25*alpha[b_idx[0]-1,b_idx[1]+1,b_idx[2]+1] + 0.25*alpha[b_idx[0]-1,b_idx[1]-1,b_idx[2]+1])
        ...
        if ("shiftz" in btype):
          ...
          if "tob" in btype:
            xzdiff =  (-0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                      + 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+2] - 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+2])
          elif "tot" in btype:
            xzdiff =  (-0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1] + 0.5*alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1]
                      + 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+2] - 0.5*alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+2])
        ...
        if ("shiftz" in btype):
          if "tob" in btype:
            xzdiff =  (0.5*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+1] - 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                       - 0.5*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]-1] + 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1])
          elif "tot" in btype:
            xzdiff =  (0.5*alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]+1] - 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1]
                       - 0.5*alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]-1] + 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]-1])
        elif ("shiftrear" in btype):
          if "tob" in btype:
            xzdiff =  (-alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+2] - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+2])
          elif "tot" in btype:
            xzdiff =  (-alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]+1] + alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1]
                      + alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]+2] - alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+2])
        elif ("shiftfront" in btype):
        ...
        if ("shifty" in btype):
          if "tore" in btype:
            xydiff =  (0.5*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+1] - 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                       - 0.5*alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]+1] + 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1])
          elif "tof" in btype:
            xydiff =  (0.5*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]-1] - 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1]
                       - 0.5*alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]-1] + 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]-1])
          ...
          if "tore" in btype:
            xydiff =  (-alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]+1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1])
          elif "tof" in btype:
            xydiff =  (-alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]-1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1]
                      + alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]-1] - alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]-1])
        elif ("shifttop" in btype):
        ...
        if ("shifty" in btype) or ("shiftbottom" in btype) or ("shifttop" in btype):
          xdiff = None # Set to None, because this term is in RHS Neumann condition
          if "tore" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1],b_idx[2]+1] - 2*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
                    + alpha[b_idx[0]+2, b_idx[1],b_idx[2]+1])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]+1] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
            #         -4*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+2] - alpha[b_idx[0]+2, b_idx[1],b_idx[2]+1]
            #       +0.5*alpha[b_idx[0]+2, b_idx[1],b_idx[2]+2] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
            xzdiff = (alpha[b_idx[0],b_idx[1],b_idx[2]+1] - alpha[b_idx[0],b_idx[1],b_idx[2]+2]
                    - alpha[b_idx[0]+1,b_idx[1],b_idx[2]+1] + alpha[b_idx[0]+1,b_idx[1],b_idx[2]+2])
          elif "tof" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1],b_idx[2]-1] - 2*alpha[b_idx[0]+1, b_idx[1],b_idx[2]-1]
                    + alpha[b_idx[0]+2, b_idx[1],b_idx[2]-1])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]-1] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]-1]
          ...
          xdiff = None # Set to None, because this term is in RHS Neumann condition
          xxdiff = (alpha[b_idx[0], b_idx[1]+1,b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
                  + alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]])
          #xxdiff = (7*alpha[b_idx[0], b_idx[1]+1,b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
          #           -4*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]] - alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]]
          #         +0.5*alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
          xydiff = (alpha[b_idx[0],b_idx[1]+1,b_idx[2]] - alpha[b_idx[0],b_idx[1]+2,b_idx[2]]
                  - alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]] + alpha[b_idx[0]+1,b_idx[1]+2,b_idx[2]])
        elif "tot" in btype or "top" in btype:
          xdiff = None # Set to None, because this term is in RHS Neumann condition
          xxdiff = (alpha[b_idx[0], b_idx[1]-1,b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]]
                  + alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]])
          ...
          #print('in else case 3, btype=', btype)
          if "tore" in btype or "rear" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1],b_idx[2]+1] - 2*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
                    + alpha[b_idx[0]+2, b_idx[1],b_idx[2]+1])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]+1] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
            #         -4*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+2] - alpha[b_idx[0]+2, b_idx[1],b_idx[2]+1]
            #       +0.5*alpha[b_idx[0]+2, b_idx[1],b_idx[2]+2] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
            xydiff = (-0.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]+2,b_idx[1]+1,b_idx[2]+1]
                    + 0.5*alpha[b_idx[0]+1,b_idx[1]-1,b_idx[2]+1] - 0.5*alpha[b_idx[0]+2,b_idx[1]-1,b_idx[2]+1])
          elif "tof" in btype or "front" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1],b_idx[2]-1] - 2*alpha[b_idx[0]+1, b_idx[1],b_idx[2]-1]
                    + alpha[b_idx[0]+2, b_idx[1],b_idx[2]-1])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]-1] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]-1]
        ...
        if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
          ...
          #print('in else case 4, btype=', btype)
          if "tob" in btype or "bottom" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1]+1,b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1]+1,b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
            #         -4*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]] - alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]]
            #      +0.5*alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
            xzdiff = (-0.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]+2,b_idx[1]+1,b_idx[2]+1]
                     + 0.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]-1] - 0.5*alpha[b_idx[0]+2,b_idx[1]+1,b_idx[2]-1])
          elif "tot" in btype or "top" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1]-1,b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]]
                    + alpha[b_idx[0]+2, b_idx[1]-1,b_idx[2]])
        ...
        if ("shifty" in btype) or ("shiftbottom" in btype) or ("shifttop" in btype):
          ...
          #print('in else case 5, btype=', btype)
          if "tore" in btype or "rear" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1],b_idx[2]+1] - 2*alpha[b_idx[0]-1, b_idx[1],b_idx[2]+1]
                    + alpha[b_idx[0]-2, b_idx[1],b_idx[2]+1])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]+1] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
        ...
        if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
          ...
          #print('in else case 6, btype=', btype)
          if "tob" in btype or "bottom" in btype:
            xxdiff = (alpha[b_idx[0], b_idx[1]+1,b_idx[2]] - 2*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]-2, b_idx[1]+1,b_idx[2]])
            #xxdiff = (7*alpha[b_idx[0], b_idx[1]+1,b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
      ...
      if "shifty" in btype:
        ...
        if ("shiftz" in btype):
          ...
          if "tol" in btype:
            yzdiff =  (-0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+1]
                      + 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+2] - 0.5*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]+2])
          elif "tori" in btype:
            yzdiff =  (-0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+1]
                      + 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+2] - 0.5*alpha[b_idx[0]-1, b_idx[1]-1,b_idx[2]+2])
        ...
        if ("shiftz" in btype):
          if "tol" in btype:
            yzdiff =  (0.5*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1] - 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                     - 0.5*alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]-1] + 0.5*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]-1])
          elif "tori" in btype:
            yzdiff =  (0.5*alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+1] - 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                     - 0.5*alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]-1] + 0.5*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]-1])
        elif ("shiftrear" in btype):
          if "tol" in btype:
            yzdiff =  (-alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+1] + alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]+2] - alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]+2])
          elif "tori" in btype:
            yzdiff =  (-alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+1] + alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+1]
                      + alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]+2] - alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]+2])
        elif ("shiftfront" in btype):
        ...
        if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
          ydiff = None # Set to None, because this term is in RHS Neumann condition
          if "tol" in btype:
            yydiff = (alpha[b_idx[0]+1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]])
            #yydiff = (7*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
            #         -4*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]] - alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]]
            #       +0.5*alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
          elif "tori" in btype:
            yydiff = (alpha[b_idx[0]-1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]])
          ...
          ydiff = None # Set to None, because this term is in RHS Neumann condition
          yydiff = (alpha[b_idx[0], b_idx[1],b_idx[2]+1] - 2*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
                  + alpha[b_idx[0], b_idx[1]+2,b_idx[2]+1])
          #yydiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]+1] + 8*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
          #           -4*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+2] - alpha[b_idx[0], b_idx[1]+2,b_idx[2]+1]
          #         +0.5*alpha[b_idx[0], b_idx[1]+2,b_idx[2]+2] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
          yzdiff = (alpha[b_idx[0],b_idx[1],b_idx[2]+1] - alpha[b_idx[0],b_idx[1],b_idx[2]+2]
                  - alpha[b_idx[0],b_idx[1]+1,b_idx[2]+1] + alpha[b_idx[0],b_idx[1]+1,b_idx[2]+2])
        elif "tof" in btype or "front" in btype:
          ydiff = None # Set to None, because this term is in RHS Neumann condition
          yydiff = (alpha[b_idx[0], b_idx[1],b_idx[2]-1] - 2*alpha[b_idx[0], b_idx[1]+1,b_idx[2]-1]
          ...
          #print('in else case 8, btype=', btype)
          if "tol" in btype or "left" in btype:
            yydiff = (alpha[b_idx[0]+1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]])
            #yydiff = (7*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
            #         -4*alpha[b_idx[0]+2, b_idx[1]+1,b_idx[2]] - alpha[b_idx[0]+1, b_idx[1]+2,b_idx[2]]
            #       +0.5*alpha[b_idx[0]+2, b_idx[1]+2,b_idx[2]] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
            yzdiff = (-0.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]+1] + 0.5*alpha[b_idx[0]+1,b_idx[1]+2,b_idx[2]+1]
                    + 0.5*alpha[b_idx[0]+1,b_idx[1]+1,b_idx[2]-1] - 0.5*alpha[b_idx[0]+1,b_idx[1]+2,b_idx[2]-1])
          elif "tori" in btype or "right" in btype:
            yydiff = (alpha[b_idx[0]-1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]]
                    + alpha[b_idx[0]-1, b_idx[1]+2,b_idx[2]])
            #yydiff = (7*alpha[b_idx[0]-1, b_idx[1],b_idx[2]] + 8*alpha[b_idx[0]-1, b_idx[1]+1,b_idx[2]]
        ...
        if ("shiftx" in btype) or ("shiftleft" in btype) or ("shiftright" in btype):
          ...
          #print('in else case 9, btype=', btype)
          if "tore" in btype or "rear" in btype:
            yydiff = (alpha[b_idx[0], b_idx[1],b_idx[2]+1] - 2*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
                    + alpha[b_idx[0], b_idx[1]+2,b_idx[2]+1])
            #yydiff = (7*alpha[b_idx[0], b_idx[1],b_idx[2]+1] + 8*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
        ...
        if ("shiftz" in btype) or ("shiftrear" in btype) or ("shiftfront" in btype):
          ...
          #print('in else case 10, btype=', btype)
          if "tol" in btype or "left" in btype:
            yydiff = (alpha[b_idx[0]+1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1]-1,b_idx[2]]
                    + alpha[b_idx[0]+1, b_idx[1]-2,b_idx[2]])
            #yydiff = (7*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1]+1,b_idx[2]]
      ...
      if "shiftz" in btype:
        ...
        zdiff = None
        if "tol" in btype:
          zzdiff = (alpha[b_idx[0]+1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
                    + alpha[b_idx[0]+1, b_idx[1],b_idx[2]+2])
          #zzdiff = (7*alpha[b_idx[0]+1, b_idx[1],b_idx[2]] + 8*alpha[b_idx[0]+1, b_idx[1],b_idx[2]+1]
          #           -4*alpha[b_idx[0]+2, b_idx[1],b_idx[2]+1] - alpha[b_idx[0]+1, b_idx[1],b_idx[2]+2]
          #         +0.5*alpha[b_idx[0]+2, b_idx[1],b_idx[2]+2] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
        elif "tori" in btype:
          zzdiff = (alpha[b_idx[0]-1, b_idx[1],b_idx[2]] - 2*alpha[b_idx[0]-1, b_idx[1],b_idx[2]+1]
                    + alpha[b_idx[0]-1, b_idx[1],b_idx[2]+2])
          ...
          zzdiff = (alpha[b_idx[0], b_idx[1]+1,b_idx[2]] - 2*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
                  + alpha[b_idx[0], b_idx[1]+1,b_idx[2]+2])
          #zzdiff = (7*alpha[b_idx[0], b_idx[1]+1,b_idx[2]] + 8*alpha[b_idx[0], b_idx[1]+1,b_idx[2]+1]
          #           -4*alpha[b_idx[0], b_idx[1]+2,b_idx[2]+1] - alpha[b_idx[0], b_idx[1]+1,b_idx[2]+2]
          #         +0.5*alpha[b_idx[0], b_idx[1]+2,b_idx[2]+2] - 10.5*alpha[b_idx[0], b_idx[1],b_idx[2]])
        elif "tot" in btype:
          zzdiff = (alpha[b_idx[0], b_idx[1]-1,b_idx[2]] - 2*alpha[b_idx[0], b_idx[1]-1,b_idx[2]+1]
                  + alpha[b_idx[0], b_idx[1]-1,b_idx[2]+2])
  ...
  print('Residual:', fw.sqrt(np.sum(lhs - args['rhs']) ** 2))
...
def solve_3d(in_tens, in_mask, max_iters, clipped_range=None, thresh_ratio=None, save_intermediate_results = False, small_eval = 5e-5, sigma=None, do_mask_open=True):
  ...
  if sigma is None:
    print("No sigma, skipping tensor smoothing")
    filt_tens = in_tens
  else:
    print("Smoothing tensors with sigma =", sigma)
    if fw_name == 'numpy':
      ...
      filt_tens = torch.from_numpy(GetNPArrayFromSITK(
                sitk.RecursiveGaussian(sitk.RecursiveGaussian(sitk.RecursiveGaussian(
                GetSITKImageFromNP(in_tens.cpu().numpy(),True), sigma=sigma,direction=0), sigma=sigma,direction=1), sigma=sigma,direction=2),True))
  ...
  precondition_tensors(tens, mask, fw)
  ...
  if fw_name == "numpy":
    g_inv = fw.copy(recon_tens)
  else:
    g_inv = fw.clone(recon_tens)
    
  g_inv[mask == 0] = iso_tens

  g = fw.linalg.inv(g_inv)

  g[fw.isnan(g)] = 0
  g_inv[fw.isnan(g_inv)] = 0

  sqrt_det_g = fw.sqrt(fw.linalg.det(g)) * mask
  sqrt_det_g[fw.isnan(sqrt_det_g)] = 1
  ...
  print('thresholded',fw.sum(sqrt_det_g > 1e4), 'pixels where sqrt_det_g > 1e4') 
  sqrt_det_g[sqrt_det_g > 1e4] = 1e4 # originally 10

  if save_intermediate_results:
    intermed_results['g'] = g
    if fw_name == "numpy":
      intermed_results['g_inv'] = fw.copy(g_inv)
    else:
      intermed_results['g_inv'] = fw.clone(g_inv)
  ...
  gradG, grad_g_inv, grad_sqrt_det_g = compute_g_derivs_new(g, g_inv, sqrt_det_g, bdry_idx, bdry_map, fw)

  grad_det_g_x_g_inv = fw.zeros((xsz,ysz,zsz,3,3,3),dtype=fw.double)
  grad_g_inv_x_det_g = fw.zeros((xsz,ysz,zsz,3,3,3),dtype=fw.double)
  det_g_x_g_inv = fw.zeros((xsz,ysz,zsz,3,3),dtype=fw.double)
  # grad_det_g_x_g_inv[i,j, gradient-direction, g-component, g-component] = gradient of sqrt_det_g .* g_inv
  # grad_g_inv_x_det_g[i,j, g-component, g-component, gradient-direction] = gradient of g_inv .* sqrt_det_g
  # det_g_x_g_inv[i,j, g-component, g-component] = sqrt_det_g .* g_inv

  # TODO switch to einsum notation for efficiency
  for ii in range(3):
    for jj in range(3):
  ...
  #v = eigenvecs[:,:,:,:,2]
  TDir = riem_vec_norm(eigenvecs[:,:,:,:,2], g, fw)

  TDir[fw.isnan(TDir)] = 0
  TDir[fw.isinf(TDir)] = 0
  ...
  gradTx_delx, gradTx_dely, gradTx_delz, gradTy_delx, gradTy_dely, gradTy_delz, gradTz_delx, gradTz_dely, gradTz_delz = compute_T_derivs_new(TDirx, TDiry, TDirz, bdry_idx, bdry_map, fw)
  ...
  first_nabla_TT = fw.zeros((xsz,ysz,zsz,3),dtype=fw.double)
  first_nabla_TT[:,:,:,0] = fw.multiply(TDirx[:,:,:,0], gradTx_delx[:,:,:]) + fw.multiply(TDiry[:,:,:,1], gradTx_dely[:,:,:]) + fw.multiply(TDirz[:,:,:,2], gradTx_delz[:,:,:])
  first_nabla_TT[:,:,:,1] = fw.multiply(TDirx[:,:,:,0], gradTy_delx[:,:,:]) + fw.multiply(TDiry[:,:,:,1], gradTy_dely[:,:,:]) + fw.multiply(TDirz[:,:,:,2], gradTy_delz[:,:,:])
  first_nabla_TT[:,:,:,2] = fw.multiply(TDirx[:,:,:,0], gradTz_delx[:,:,:]) + fw.multiply(TDiry[:,:,:,1], gradTz_dely[:,:,:]) + fw.multiply(TDirz[:,:,:,2], gradTz_delz[:,:,:])
  ...
  christoffel=fw.zeros((xsz,ysz,zsz,3,3,3),dtype=fw.double)
  second_nabla_TT = fw.zeros((xsz,ysz,zsz,3),dtype=fw.double)
  # TODO consider switching to einsum notation for efficiency
  for k in range(3):
    for p in range(3):
      for q in range(3):
  ...
  sqrt_det_nabla_TT = fw.zeros((xsz,ysz,zsz,3),dtype=fw.double)
  ...
  grad_nabla_TT, grad_sqrt_det_nabla_TT = compute_nabla_derivs_new(nabla_TT, sqrt_det_nabla_TT, bdry_idx, bdry_map, fw)
  ...
  if save_intermediate_results:
    ...
    if fw_name == "numpy":
      intermed_results['rhs_b4_neumann'] = fw.copy(rhs)
    else:
      intermed_results['rhs_b4_neumann'] = fw.clone(rhs)
  ...
  neumann_terms = neumann_conditions_rhs(nabla_TT, g, grad_det_g_x_g_inv, grad_g_inv_x_det_g, det_g_x_g_inv, sqrt_det_g, bdry_idx, bdry_map, fw)
  rhs -= neumann_terms
  ...
  if fw_name == 'numpy':
    ...
    # Right now gmres is numpy only
    # Look at xitorch as possible torch-based gmres solver
    args['det_g_x_g_inv'] = det_g_x_g_inv.cpu().numpy()
    args['grad_det_g_x_g_inv'] = grad_det_g_x_g_inv.cpu().numpy()
    args['grad_g_inv_x_det_g'] = grad_g_inv_x_det_g.cpu().numpy()
    args['sqrt_det_g'] = sqrt_det_g.cpu().numpy()
    args['mask'] = mask.cpu().numpy()
    args['xsz'] = xsz
    args['ysz'] = ysz
    args['zsz'] = zsz
    args['rhs'] = rhs.cpu().numpy()
    args['bdry_idx'] = bdry_idx
    args['bdry_map'] = bdry_map
    args['fw'] = np
  ...
  #TODO look for torch linear operator options
  AxLO = LinearOperator((mask_shape[0], mask_shape[0]), Ax_args)

  if fw_name == 'numpy':
    b = rhs[mask > 0]
  else:
    b = rhs[mask > 0].cpu().numpy()
  ...
  alpha = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  if fw_name == 'numpy':
    alpha[mask > 0] = x
  else:
    alpha[mask > 0] = torch.from_numpy(x)
...
####################################
  ...
  if fw_name == "numpy":
    clipped_alpha = fw.copy(alpha)
  else:
    clipped_alpha = fw.clone(alpha)

  clipped_alpha[alpha < clipped_range[0]] = clipped_range[0]
  clipped_alpha[alpha > clipped_range[1]] = clipped_range[1]

  # final Ax
  Ax_final_noclip = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  Ax_final_clip = fw.zeros((xsz,ysz,zsz),dtype=fw.double)
  if fw_name == 'numpy':
    Ax_final_noclip[mask > 0] = AxLO(x)
    Ax_final_clip[mask > 0] = AxLO(clipped_alpha[mask > 0])
  else:
    Ax_final_noclip[mask > 0] = torch.from_numpy(AxLO(x))
    Ax_final_clip[mask > 0] = torch.from_numpy(AxLO(clipped_alpha[mask > 0]))

  res_img_noclip = fw.abs(Ax_final_noclip-rhs)
  res_img_clip = fw.abs(Ax_final_clip-rhs)
  res_noclip = fw.sqrt(fw.sum(res_img_noclip**2))
  res_clip = fw.sqrt(fw.sum(res_img_clip**2))
  ...
  #else:
    # Don't need to clip alpha if we've filtered sufficiently well.  While min and max values
    # may be a bit high, they should still stay realistic.  
  #  return(alpha, scaled_tensors_noclip, mask, iters.rks, intermed_results)
# end solve_3d
