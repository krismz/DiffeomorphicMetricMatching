...
def geodesicpath_3d_torch(tensor_field, mask_image, start_coordinate, initial_velocity, delta_t=0.15, metric='withoutscaling', iter_num=18000, filename = '', both_directions=False):
  ...
  geodesicpath_points_x = torch.zeros((iter_num))
  geodesicpath_points_y = torch.zeros((iter_num))
  geodesicpath_points_z = torch.zeros((iter_num))
  eps11 = tensor_field[0, :, :]
  eps12 = tensor_field[1, :, :]
  eps13 = tensor_field[2, :, :]
  eps22 = tensor_field[3, :, :]
  ...
  gamma = torch.zeros((iter_num,3))
  gamma_dot = torch.zeros((iter_num,3))
  gamma_ddot = torch.zeros((iter_num,3))
  gamma[0, :] = start_coordinate
  gamma_dot[0, :] = init_v
  ...
  if both_directions:
    geodesicpath_points_x = torch.concatenate((geodesicpath_points_x[::-1], back_x))
    geodesicpath_points_y = torch.concatenate((geodesicpath_points_y[::-1], back_y))
    geodesicpath_points_z = torch.concatenate((geodesicpath_points_z[::-1], back_z))
