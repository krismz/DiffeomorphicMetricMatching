...
def fwd_diff_2d(a, fw=np):
  # return the forward diff in the x and the y direction
  fwd_x = fw.zeros_like(a)
  fwd_y = fw.zeros_like(a)
  fwd_x[:-1,:] = a[1:,:] - a[:-1,:]
  fwd_y[:,:-1] = a[:,1:] - a[:,:-1]
  return(fwd_x, fwd_y)

def back_diff_2d(a, fw=np):
  # return the backward diff in the x and the y direction
  back_x = fw.zeros_like(a)
  back_y = fw.zeros_like(a)
  back_x[1:,:] = a[1:,:] - a[:-1,:]
  back_y[:,1:] = a[:,1:] - a[:,:-1]
  return(back_x, back_y)

def gradient_idx_2d(a, idx, fw=np):
  ...
  grad_x = fw.zeros_like(a)
  grad_y = fw.zeros_like(a)
...
def gradient_mask_2d(img, mask):
  ...
  gradX, gradY = gradient_bdry_2d(img, bdry_idx, bdry_map, fw)
  return(gradX, gradY)
# def end gradient_mask_2d

def gradient_bdry_2d(img, bdry_idx, bdry_map, fw=np):

  xsz = img.shape[0]
  ysz = img.shape[1]
  gradX = fw.zeros((xsz,ysz),dtype=fw.double)
  gradY = fw.zeros((xsz,ysz),dtype=fw.double)
...
def eigv_gradx_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_grady_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  # using only terms to the right
  # called left, because used for case when left direction (i-1) is out of bounds
  leftx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  lefty_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, right1) < 0:
      right1 = -right1
    if fw.dot(pix, right2) < 0:
      right2 = -right2
...
def eigv_right_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  # using only terms to the left
  # called right, because used for case when right direction (i+1) is out of bounds
  rightx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  righty_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, left1) < 0:
      left1 = -left1
    if fw.dot(pix, left2) < 0:
      left2 = -left2
...
def eigv_bottom_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  # using only terms to the top
  # called bottom, because used for case when bottom direction (j-1) is out of bounds
  bottomx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  bottomy_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, top1) < 0:
      top1 = -top1
    if fw.dot(pix, top2) < 0:
      top2 = -top2
...
def eigv_top_idx_2d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  # using only terms to the bottom
  # called top, because used for case when top direction (j+1) is out of bounds
  topx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  topy_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, bottom1) < 0:
      bottom1 = -bottom1
    if fw.dot(pix, bottom2) < 0:
      bottom2 = -bottom2
...
def eigv_bottomleft_idx_2d(eigvecs, idx, fw=np):
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, right) < 0:
      right = -right
    if fw.dot(pix, top) < 0:
      top = -top
    if fw.dot(pix, topright) < 0:
      topright = -topright
...
def eigv_topleft_idx_2d(eigvecs, idx, fw=np):
  # WARNING! bottomleft, topleft, bottomright, topright greatly reduce performance for analytic annulus case!
  # USE AT OWN RISK!!!
  # Return the gradient of each component of the eigenvector taken in the x and y directions
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, right) < 0:
      right = -right
    if fw.dot(pix, bottom) < 0:
      bottom = -bottom
    if fw.dot(pix, bottomright) < 0:
...
def eigv_bottomright_idx_2d(eigvecs, idx, fw=np):
  # WARNING! bottomleft, topleft, bottomright, topright greatly reduce performance for analytic annulus case!
  # USE AT OWN RISK!!!
  # Return the gradient of each component of the eigenvector taken in the y direction
  # using only terms to the top
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, left) < 0:
      left = -left
    if fw.dot(pix, top) < 0:
      top = -top
    if fw.dot(pix, topleft) < 0:
      topleft = -topleft
...
def eigv_topright_idx_2d(eigvecs, idx, fw=np):
  # WARNING! bottomleft, topleft, bottomright, topright greatly reduce performance for analytic annulus case!
  # USE AT OWN RISK!!!
  # Return the gradient of each component of the eigenvector taken in the x direction
  # using only terms to the left
...
def bottom_diff_3d(a, fw=np):
  # return the derivative in the y direction using only terms to the top (increasing y direction)
  # called bottom, because used for case when bottom direction (j-1) is out of bounds
  bot_x = fw.zeros_like(a)
...
def top_diff_3d(a, fw=np):
  # return the derivative in the y direction using only terms to the bottom (decreasing y direction)
  # called top, because used for case when top direction (j+1) is out of bounds
  top_x = fw.zeros_like(a)
...
def gradient_mask_3d(img, mask, modify_mask=True):
  ...
  gradX, gradY, gradZ = gradient_bdry_3d(img, bdry_idx, bdry_map, fw)
...
def eigv_gradx_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_grady_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_gradz_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(front, rear) < 0:
      if fw.dot(front, pix) < 0:
        front = -front
      else:
        rear = -rear
...
def eigv_gradx_shiftbottom_idx_3d(eigvecs, idx, fw=np):
...
def eigv_gradx_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the y direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_gradx_shiftrear_idx_3d(eigvecs, idx, fw=np):
...
def eigv_gradx_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the z direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_shiftbottom_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the y direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the y direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_shiftrear_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the z direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the z direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_right_shiftbottom_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the y direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_right_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the y direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_right_shiftrear_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the z direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_right_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction, shifted in the z direction
  gradx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_grady_shiftleft_idx_3d(eigvecs, idx, fw=np):
...
def eigv_grady_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_grady_shiftrear_idx_3d(eigvecs, idx, fw=np):
...
def eigv_grady_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the z direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_bottom_shiftleft_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_bottom_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_bottom_shiftrear_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_bottom_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_top_shiftleft_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_top_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_top_shiftrear_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_top_shiftfront_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction shifted in the x direction
  gradx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_gradz_shiftleft_idx_3d(eigvecs, idx, fw=np):
...
def eigv_gradz_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the x direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_gradz_shiftbottom_idx_3d(eigvecs, idx, fw=np):
...
def eigv_gradz_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the y direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_rear_shiftleft_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the x direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, front) < 0:
      front = -front
    if fw.dot(pix, front2) < 0:
      front2 = -front2
...
def eigv_rear_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the x direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_rear_shiftbottom_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the y direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_rear_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the y direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_front_shiftleft_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the x direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  ...
  for ii in range(len(idx[0])):
    ...
    if fw.dot(pix, rear) < 0:
      rear = -rear
    if fw.dot(pix, rear2) < 0:
      rear2 = -rear2
...
def eigv_front_shiftright_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the x direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_front_shiftbottom_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the y direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_front_shifttop_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction shifted in the y direction
  gradx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  grady_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  gradz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_left_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  # using only terms to the right
  # called left, because used for case when left direction (i-1) is out of bounds
  leftx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  lefty_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  leftz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_right_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the x direction
  # using only terms to the left
  # called right, because used for case when right direction (i+1) is out of bounds
  rightx_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  righty_delx = fw.zeros(len(idx[0]),dtype=fw.double)
  rightz_delx = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_bottom_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  # using only terms to the top
  # called bottom, because used for case when bottom direction (j-1) is out of bounds
  bottomx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  bottomy_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  bottomz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_top_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the y direction
  # using only terms to the bottom
  # called top, because used for case when top direction (j+1) is out of bounds
  topx_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  topy_dely = fw.zeros(len(idx[0]),dtype=fw.double)
  topz_dely = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_rear_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction
  # using only terms to the front
  # called rear, because used for case when rear direction (k-1) is out of bounds
  rearx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  reary_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  rearz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
...
def eigv_front_idx_3d(eigvecs, idx, fw=np):
  # Return the gradient of each component of the eigenvector taken in the z direction
  # using only terms to the rear
  # called front, because used for case when front direction (k+1) is out of bounds
  frontx_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  fronty_delz = fw.zeros(len(idx[0]),dtype=fw.double)
  frontz_delz = fw.zeros(len(idx[0]),dtype=fw.double)
