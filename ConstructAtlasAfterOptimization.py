...
if __name__ == "__main__":
    ...
    subj_type = 'controls' # 'cases','controls', or 'both'
    # file_name = []
    ...
    outc = []
    for offset in offs:
        if (subj_type == 'both') or (subj_type == 'cases'):
          outc.append(cases[offset])
        #outc.append(cases[offset])
        if (subj_type == 'both') or (subj_type == 'controls'):
    ...
    #     balance the background and subject by rescaling
        ...
        diffeo_list.append(torch.from_numpy(sio.loadmat(f'{output_dir}/{subj}_{session}_phi_inv.mat')['diffeo']).double().to(device))
    ...
    WriteTensorNPArray(np.transpose(atlas_lin,(3,2,1,0)), f'{output_dir}/final_atlas_tens.nhdr')
    WriteScalarNPArray(np.transpose(atlas_mask.cpu(),(2,1,0)), f'{output_dir}/final_atlas_mask.nhdr')
