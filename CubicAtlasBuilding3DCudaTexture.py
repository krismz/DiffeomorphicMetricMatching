...
# the phi here is identity
    ...
    print(sigma*E1, E2*0.5, E3, 'DIFFERENT THAN HDAI VERSION')
    return sigma*E1 + E2*0.5 + E3
   
    # Following was hdai version
    #return E1 + E2*2.5e2 + E3*0#1.5e-9

def energy_ebin_logmap(phi, g0, g1, f0, f1, i0, i1, zero, exp_g0, u, sigma, sigmau, dim, mask,metWeight=None,imWeight=None,expWeight=None): 
#     input: phi.shape = [3, h, w, d]; g0/g1/f0/f1.shape = [h, w, d, 3, 3]; sigma/dim = scalar; mask.shape = [1, h, w, d]
#     output: scalar
# the phi here is identity
    met_size = g0.size()
    ...
    phi_star_f1 = phi_pullback(phi, f1)# the compose operation in this step uses a couple of thousands MB of memory
    phi_star_i1 = compose_function(i1.unsqueeze(0), phi, mask, 0).squeeze()# the compose operation in this step uses a couple of thousands MB of memory
    invexp_g1 = inv_RieExp_extended(g1.reshape(-1,*met_size[-2:]), exp_g0.reshape(-1,*met_size[-2:]), 1.0/dim).reshape(*met_size)
    
    E1 = Squared_distance_Ebin(f0, phi_star_f1, 1./dim, mask)
    E2 = Squared_distance_Ebin(g0, phi_star_g1, 1./dim, mask)
    E3 = torch.sum((i0 - phi_star_i1) ** 2)
    E4 = energy_u(torch.zeros_like(u), u, mask)
    E5 = energy_u(invexp_g1, zero, mask)
    print(E1, E2, E3, E4, E5, 'prescaled')
    if metWeight is None:
      metWeight = 2.5e2
    if imWeight is None:
      imWeight = 0.3e9
    if expWeight is None:
      expWeight = 2.5e2
    
    print(sigma*E1, E2*metWeight, E3*imWeight, sigmau*E4, E5*expWeight, 'after scaling')
    return sigma*E1 + E2*metWeight + E3*imWeight + sigmau*E4 + E5*expWeight
   
def energy_u(u0, u1, mask): 
#     input: u0/u1.shape = [h, w, d, 3, 3]; mask.shape = [1, h, w, d]
#     output: scalar
    E = torch.einsum("ijk...,lijk->", (u0 - u1) ** 2, mask.unsqueeze(0))
    return E
...
def metric_matching_logmap(gi, gm, ii, im, height, width, depth, ith_mask, mask, iter_num, epsilon, epsilonu, sigma, sigmau, dim, met_weight=None, img_weight=None, exp_weight=None, uinit=None):
    ...
    if uinit is not None:
      ...
      u = torch.zeros_like(gi).to(cuda_dev)
    zero = torch.zeros_like(gi).to(cuda_dev)
    zero.requires_grad_()
    ...
    exp_phi_acts_g0 = Rie_Exp_extended(phi_actsg0.reshape(-1,*met_size[-2:]), u.reshape(-1,*met_size[-2:]), 1.0/dim, 1.0).reshape(*met_size)
    ...
    for j in range(iter_num):
        ...
        E = energy_ebin_logmap(idty, phi_actsg0, gm.to(cuda_dev), phi_actsf0, f1, phi_actsi0, im.to(cuda_dev), zero, exp_phi_acts_g0, u, sigma, sigmau, dim, mask.to(cuda_dev), 
                        met_weight, img_weight, exp_weight) 
        print(E.item())
        if torch.isnan(E):
            raise ValueError('NaN error')
        E.backward()
        v = - laplace_inverse(idty.grad)
        u = u + epsilonu*zero.grad
        with torch.no_grad():
            #print('metric_matching v NaN?', v.isnan().any())

            print('iter', j, 'metric_matching, energy is', E.item(), ', epsilon u is', epsilonu, 'and epsilon is', epsilon)
    ...
    gu = Rie_Exp_extended(gi.reshape(-1,*met_size[-2:]), u.reshape(-1,*met_size[-2:]), 1.0/dim, 1.0).reshape(*met_size)
    #return gi, ii.squeeze(), phi, phi_inv
    return gi, ii.squeeze(), gu, ith_mask, phi, phi_inv, u
...
if __name__ == "__main__":
    ...
    for sim, num_subjs in zip(sims, num_subjs_in_sim):
      ...
      tensor_lin_list, tensor_met_list, tensor_exp_met_list, mask_list, mask_thresh_list, fa_list, img_list, brain_mask_list, logmap_list = [], [], [], [], [], [], [], [], []
      ...
      for s in range(len(cases)):
          ...
          if resume==False:
              ...
              logmap_list.append(torch.zeros_like(tensor_met_zeros).to(cuda_dev))
          else:
              ...
              logmap_lin = np.transpose(ReadTensors(f'{outdir}/{prefix}{subj}_{start_iter-1}_logmap.nhdr'),(2,1,0,3))
              logmap = torch.zeros(height,width,depth,3,3,dtype=torch.float64).to(device)
              logmap[:,:,:,0,0] = logmap_lin[:,:,:,0]
              logmap[:,:,:,0,1] = logmap_lin[:,:,:,1]
              logmap[:,:,:,0,2] = logmap_lin[:,:,:,2]
              logmap[:,:,:,1,0] = logmap_lin[:,:,:,1]
              logmap[:,:,:,1,1] = logmap_lin[:,:,:,3]
              logmap[:,:,:,1,2] = logmap_lin[:,:,:,4]
              logmap[:,:,:,2,0] = logmap_lin[:,:,:,2]
              logmap[:,:,:,2,1] = logmap_lin[:,:,:,4]
              logmap[:,:,:,2,2] = logmap_lin[:,:,:,5]

              logmap_list.append(logmap)
          met_energy_list.append([])    
          img_energy_list.append([])    
          tensor_exp_met_list.append(Rie_Exp_extended(tensor_met_list[s].reshape(-1,3,3), logmap_list[s].reshape(-1,3,3), 1.0/3).reshape(height,width,depth,3,3)) 
      ...
      #dim, sigma, epsilon, inner_iter_num = 3., 0.5, 5.e-5, 2 # epsilon = 1e-7 too small?, 5e-3 may be slightly too big, 5e-4 is too small.  1e-3?
      dim, inner_iter_num = 3., 2 # try matching settings from image only atlas building
      sigma, sigmau = 1, 1
      epsilon, epsilonu = 1.0e-5, 1.0e-2
      #met_weight, img_weight, exp_weight = 0.25, 1.0, 0.25 # Diverges for cubics
      met_weight, img_weight, exp_weight = 0.05, 1.0, 0.005
      ...
      for i in tqdm(range(start_iter, start_iter+iter_num)):
        G = torch.stack(tuple(tensor_exp_met_list))
        ...
        for s in range(sample_num):
          ...
          tensor_met_list[s], img_list[s], tensor_exp_met_list[s], mask_list[s], phi, phi_inv, logmap_list[s] = metric_matching_logmap(tensor_met_list[s], atlas, img_list[s], mean_img, height, width, depth, mask_list[s], mask_union, inner_iter_num, epsilon, epsilonu, sigma, sigmau, dim, met_weight, img_weight, exp_weight, logmap_list[s])
        ...
        if i%200==0:
            ...
            for s in range(sample_num):
                ...
                logmap_lin =  np.zeros((6,height,width,depth))
                logmap_lin[0] = logmap_list[s][:,:,:,0,0].cpu()
                logmap_lin[1] = logmap_list[s][:,:,:,0,1].cpu()
                logmap_lin[2] = logmap_list[s][:,:,:,0,2].cpu()
                logmap_lin[3] = logmap_list[s][:,:,:,1,1].cpu()
                logmap_lin[4] = logmap_list[s][:,:,:,1,2].cpu()
                logmap_lin[5] = logmap_list[s][:,:,:,2,2].cpu()
    
                WriteTensorNPArray(np.transpose(logmap_lin,(3,2,1,0)),f'{outdir}/{prefix}{subj}_{i}_logmap.nhdr')
      ...
      for s in range(sample_num):
          ...
          WriteTensorNPArray(np.transpose(logmap_lin,(3,2,1,0)),f'{outdir}/{prefix}{subj}_logmap.nhdr')                        
