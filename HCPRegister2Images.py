...
def energy_ebin(phi, phi0, f0, f1, i0, i1, sigma, dim, mask): 
...
def metric_matching(ii, im, height, width, depth, ith_mask, mask, iter_num, epsilon, sigma, dim):
    ...
    energy_prev = 2 * energy_ebin(idty, phi_inv, phi_actsf0, f1, phi_actsi0, im.to(cuda_dev), sigma, dim, mask.to(cuda_dev)) # Double up so that first iteration does go down
    ...
    for j in range(iter_num):
        ...
        E = energy_ebin(idty, phi_inv, phi_actsf0, f1, phi_actsi0, im.to(cuda_dev), sigma, dim, mask.to(cuda_dev)) 
        print(E.item())
    ...
    torch.cuda.empty_cache()
    return ii.squeeze(), ith_mask, phi, phi_inv, energy
...
if __name__ == "__main__":
    ...
    for s in range(len(cases)-1):
        for t in range(s+1,len(cases)):
          subj1 = cases[s]
          subj2 = cases[t]
  
          phi_inv = get_idty(height, width, depth,device=device)
          phi = get_idty(height, width, depth,device=device)

          mask_union = torch.clone(mask_list[s])
          mask_union[mask_list[t]>0] = 1
    
          print(f'Registering {subj1} and {subj2}, for iter_num = {iter_num}, epsilon = {epsilon}')

          #img_energy_list[s].append(torch.sum((img_list[s] - img_list[t])**2).item())
          
          img_s_reg_to_t, mask_s_reg_to_t, phi, phi_inv, energy = metric_matching(img_list[s], img_list[t], height, width, depth, mask_list[s], mask_union, iter_num, epsilon, sigma, dim)

          img_energy_list[s] = energy

          sio.savemat(f'{output_dir}/{subj1}_{subj2}_phi_inv.mat', {'diffeo': phi_inv.cpu().detach().numpy()})
          sio.savemat(f'{output_dir}/{subj1}_{subj2}_phi.mat', {'diffeo': phi.cpu().detach().numpy()})
          sio.savemat(f'{output_dir}/{subj1}_{subj2}_img_energy.mat', {'energy': img_energy_list[s]})
          WriteScalarNPArray(np.transpose(img_s_reg_to_t.cpu(),(2,0,1)), f'{output_dir}/{subj1}_{subj2}_t1.nhdr')
          #break
        # end for t
        #break
    # end for s
