...
def view_3d_tensors(tens, mask=None, img=None, paths=None, labels=None, xrng=None, yrng=None, zrng=None, stride=5, scale=None, viewer=None, has_component_data=False, num_tube_pts=500, tube_radius=0.5, colors=[]):
  ...
  # wrapping in do_view in case we want to add an interaction to update xrng,yrng,zrng  
  def do_view(tens,mask,img,paths,labels,xrng,yrng,zrng,stride,scale,colors,viewer=None):
    glyphs = tensors_to_mesh(tens, mask, xrng, yrng, zrng, stride, scale)
    #glyphs = {} # Doesn't work because tensors_to_mesh returns an array of glyphs
    #glyphs['tensors'] = tensors_to_mesh(tens, mask, xrng, yrng, zrng, stride, scale)
    if paths is not None:
      for p, label in zip(paths,labels):
        tube = path_to_tube(p[0], p[1], p[2], num_tube_pts, tube_radius)
        glyphs.append(tube)
  ...
  viewer = do_view(tens, mask, img, paths, labels, xrng, yrng, zrng, stride, scale, colors, viewer)
