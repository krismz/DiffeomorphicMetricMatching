...
def compute_geodesics(atlas_tens_4_path, atlas_mask, seed_mask, end_mask, offset, fileprefix, pool):
  ...
  atlas_geos = []
  get_paths_tens = partial(get_paths, tens=atlas_tens_4_path, seed_mask=seed_mask, end_mask=end_mask,
...
if __name__ == "__main__":
  ...
  if do_atlas:
    ...
    outatlasdirs.append('/usr/sci/projects/HCP/Kris/NSFCRCNS/TestResults/MELBAResults/atlases/BrainAtlasUkfBallImgMetDirectRegSept10/')
    ...
    for atlas_geo_dir, atlasdir, atlasname, outatlasdir in zip(atlas_geo_dirs, atlasdirs, atlasnames, outatlasdirs):
      ...
      for rmask in region_masks:
        ...
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask1_start, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{rmask}_hemi1_', pool)
        ars.append(ar)
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask2_start, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{rmask}_hemi2_', pool)
        ars.append(ar)
      for smask in single_masks:
        mask = ReadScalars(outatlasdir + f'{smask}.nhdr')
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{smask}_', pool)
        ars.append(ar)
    ...
    if subj_atlas_space:
      ...
      out_atlas_dir = f'/usr/sci/projects/HCP/Kris/NSFCRCNS/TestResults/MELBAResults/atlases/BrainAtlasUkfBallImgMetDirectRegSept10/'
    else:
      atlas_geo_dir = f'/usr/sci/projects/HCP/Kris/NSFCRCNS/TestResults/MELBAResults/b{bval}_UKF_orig_scaled_tens_rreg/'
      out_atlas_dir = '/usr/sci/projects/HCP/Kris/NSFCRCNS/TestResults/MELBAResults/atlases/BrainAtlasUkfBallImgMetDirectRegSept10/subjspace/'
    #region_masks.append('CST')
    #region_masks.append('Cingulum_Cor')
    #region_masks.append('Cingulum_Sag')
    #region_masks.append('SLF')
    ...
    ars = []
    for subj in subjs:
      atlasname = subj + f'_{bval}_'

      if subj_atlas_space:
        atlas_tens = ReadTensors(f'{out_atlas_dir}/{subj}_scaled_orig_tensors_rreg_atlas_space.nhdr')
        #atlas_mask = ReadScalars(f'{outdir}/{subj}_filt_mask.nhdr')
        atlas_mask = ReadScalars(f'{out_atlas_dir}/{subj}_FA_mask_rreg_atlas_space.nhdr')
        end_mask = ReadScalars(f'{out_atlas_dir}/{subj}_FA_mask_0.20_rreg_atlas_space.nhdr')
        atlas_tens_4_path = np.transpose(atlas_tens,(3,0,1,2))
      else:
      ...
      for rmask in region_masks:
        if subj_atlas_space:
          mask1_start = ReadScalars(out_atlas_dir + f'/{rmask}_hemi1_start.nhdr')
          mask2_start = ReadScalars(out_atlas_dir + f'/{rmask}_hemi2_start.nhdr')
        else:
          mask1_start = ReadScalars(out_atlas_dir + f'/{subj}_{rmask}_hemi1_start.nhdr')
          mask2_start = ReadScalars(out_atlas_dir + f'/{subj}_{rmask}_hemi2_start.nhdr')
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask1_start, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{rmask}_hemi1_', pool)
        ars.append(ar)
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask2_start, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{rmask}_hemi2_', pool)
        ars.append(ar)
      for smask in single_masks:
        if subj_atlas_space:
          mask = ReadScalars(out_atlas_dir + f'{smask}.nhdr')
        else:
          mask = ReadScalars(out_atlas_dir + f'{subj}_{smask}.nhdr')
        ar = compute_geodesics(atlas_tens_4_path, atlas_mask, mask, end_mask, offs, f'{atlas_geo_dir}{atlasname}_geos_{smask}_', pool)
        ars.append(ar)  
    for ar in ars:
      ar.wait()
