...
if __name__ == "__main__":
  ...
  for cc in cases:
    try:
      ...
      t1_file = 't1_to_reft1_affreg.nhdr'
      ...
      WriteTensorNPArray(out_tens_tri, out_prefix + f'_thresh_{thresh_ratio}_tensors_rreg.nhdr')
      WriteTensorNPArray(in_tens, out_prefix + '_orig_tensors_rreg.nhdr')
      WriteScalarNPArray(out_mask, out_prefix + '_filt_mask_rreg.nhdr')
      WriteScalarNPArray(alpha, out_prefix + '_alpha_rreg.nhdr')
      if t1_file:
        WriteScalarNPArray(in_T1, out_prefix + '_T1_affreg.nhdr')
        WriteScalarNPArray(out_T1, out_prefix + '_T1_flip_y_affreg.nhdr')
      if save_intermediate_results:
        WriteTensorNPArray(scaled_tens_tri, out_prefix + f'_scaled_tensors_rreg.nhdr')
